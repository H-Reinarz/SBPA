...
def _radial_lbp(double[:, ::1] image,
                          int P, float RO, float RI, char method='D'):
    ...
    # local position of OUTER texture elements 
    rro = - RO * np.sin(2 * np.pi * np.arange(P, dtype=np.double) / P)
    cco = RO * np.cos(2 * np.pi * np.arange(P, dtype=np.double) / P)
    cdef double[::1] rpo = np.round(rro, 5)
    cdef double[::1] cpo = np.round(cco, 5)

    # local position of INNER texture elements 
    rri = - RI * np.sin(2 * np.pi * np.arange(P, dtype=np.double) / P)
    cci = RI * np.cos(2 * np.pi * np.arange(P, dtype=np.double) / P)
    cdef double[::1] rpi = np.round(rri, 5)
    cdef double[::1] cpi = np.round(cci, 5)
    
    # pre-allocate arrays for computation
    cdef double[::1] texture_o = np.zeros(P, dtype=np.double)
    cdef double[::1] texture_i = np.zeros(P, dtype=np.double)
    ...
    with nogil:
        for r in range(image.shape[0]):
            for c in range(image.shape[1]):
                for i in range(P):
                    #OUTER POINTS
                    texture_o[i] = bilinear_interpolation(&image[0, 0], rows, cols,
                                                        r + rpo[i], c + cpo[i],
                                                        'C', 0)
                    #INNER POINTS
                    texture_i[i] = bilinear_interpolation(&image[0, 0], rows, cols,
                                                        r + rpi[i], c + cpi[i],
                                                        'C', 0)

                # signed / thresholded texture
                for i in range(P):
                    if texture_o[i] - texture_i[i] >= 0:
                        signed_texture[i] = 1
                    else:
                        signed_texture[i] = 0
