...
def _angular_lbp(double[:, ::1] image,
    ...
    cdef double[::1] angular_diff = np.zeros(P, dtype=np.double)
        ...
        for r in range(image.shape[0]):
            for c in range(image.shape[1]):
                ...
                for i in range(P):
                    ...
                    if i == 0:
                        angular_diff[i] = texture[0] - texture[P-1]                    
                    else:
                        angular_diff[i] = texture[i] - texture[i-1]
