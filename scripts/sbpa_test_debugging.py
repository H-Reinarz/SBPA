...
result = Graph.produce_cluster_image('cluster')

out = color.label2rgb(result, image, kind='avg')
out = mark_boundaries(out, result, (0, 0, 0))
f, ax = plt.subplots(figsize=(10, 10))
ax.imshow(mark_boundaries(image, result));

from SBPA.visual import plot_node_attribute
f, ax = plt.subplots(figsize=(10, 10))

ax.imshow(image)
ax.imshow(out, alpha = .4)
ax.imshow(result, cmap=plt.cm.spectral, alpha=.2)